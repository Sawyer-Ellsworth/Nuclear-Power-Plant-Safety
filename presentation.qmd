...
-   This project looks at two data sets.
-   The first contains information about nuclear power generation in MWh at most commercial nuclear power plants in the United States.
-   The second contains information about safety metrics at these plants, such as scrams and coolant leaks.
-   The goal of this project is to analyze the relationship between power generation and safety metrics at these plants from 2007-2009.

## Finding data

-   Power generation data was taken from the U.S. Energy Information Administration (EIA) with each year being a seperate data set.
-   Safety metrics data was obtained from the Nuclear Regulatory Commission (NRC) with each site being a separate pdf.

## Making Data

-   Safety metric data was in pdf format so it had to be extracted and transformed into a usable format, csv.
-   To do this I used Gemini to coagulate the data from the pdfs into a single csv file.
-   This took several attempts of writing custom prompts in notbook lm to take the pdfs and extract the relevant data.
 - After that I just joined the data sets together

## The Safety Score
To compare the safety score to the amount of power generated, I created a new variable to give to ggplot.
...
## Creating the App and Using it in Quarto
...
server <- function(input, output, session) {
  ...
  output$safety_plot <- renderPlot({
    ...
    if (input$stat == "power_vs_safety") {
      ...
      # Correlation plot: Safety vs Power
      ggplot(df, aes(
        x = power_generated,
        y = overall_safety_score
      )) +
